% script to fit the Sutton K1 with combined response model
% 
% Note - for this model to work, fitModel needs to be edits so that r.traj
% is output from perceptual function. Lines 426:
%   [traj, infStates] = prc_fun(r, ptrans_prc, 'trans');
%   r.traj = traj;
...
% load models
[prc_config, obs_config] = cbbu_SuttonK1_comb_obs_config;
...
ptps = [1:44, 95, 96, 97, 99];
model_fits = cell(numel(ptps),1);
parfor i = 1:numel(ptps)

    sub_i = ptps(i);

    % sub directory
    sub_dir = ['..\task_data\sub-', num2str(sub_i, '%02i'), '\'];

    % preallocate est
    est = [];

    % load data
    run1_fname = dir(fullfile(sub_dir, ['sub-', num2str(sub_i, '%02i'), '_facehouse-MRI_run1*.mat'])).name;
    run2_fname = dir(fullfile(sub_dir, ['sub-', num2str(sub_i, '%02i'), '_facehouse-MRI_run2*.mat'])).name;
...
save('cbbu_SuttonK1_comb_obs_model_fits.mat', 'model_fits', 'ptps');
