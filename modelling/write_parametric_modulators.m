...
d = importdata('dual_stream\cbbu_uHGF_3level_comb_obs2_model_fits.mat');
fits = d.model_fits;
fits_ptps = d.ptps;
...
for iSub = 1:numel(ptps)
    
    sub = ptps(iSub);
    sub_fit = fits{fits_ptps == sub};
    
    
    for iRun = 1:2
        % load subject data
        if exist('data_dir', 'var')
            d = dir([data_dir, '\sub-', num2str(sub, '%02i'), '\task_fmri\sub-', num2str(sub, '%02i'), '_facehouse-MRI_run', num2str(iRun), '*']);
        elseif exist('BIDS_root', 'var')
            d = dir([BIDS_root, '\sub-', num2str(sub, '%02i'), '\beh\sub-', num2str(sub, '%02i'), '_facehouse-MRI_run', num2str(iRun), '*']);
        end
        ...
        e2      = sub_fit.traj.epsi(:,2); % e2
        e3      = sub_fit.traj.epsi(:,3); % e3
        psi2    = sub_fit.traj.psi(:,2);  % psi2
        psi3    = sub_fit.traj.psi(:,3);  % psi3
        mu3     = sub_fit.traj.mu(:,3);   % mu3
        ...
        % save
        mkdir([BIDS_root, '\sub-', num2str(sub, '%02i'), '\GLM\uHGF_3level_comb_obs2_parametric_modulators']);
        save([BIDS_root, '\sub-', num2str(sub, '%02i'), '\GLM\uHGF_3level_comb_obs2_parametric_modulators\run_', num2str(iRun), '_parametric_modulators.mat'], 'pm');        
    end
end
